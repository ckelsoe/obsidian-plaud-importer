...
Like the subfolder setting, this applies to **new imports**; existing notes are renamed only when you rename them yourself or when the recording changes in Plaud (see [Renaming recordings](#renaming-recordings)). One exception: a note whose name still contains `#` or `^` from an older import moves to the new name the next time it is re-imported with overwrite on.
