...
export default class PlaudImporterPlugin extends Plugin {
	...
	// The pre-expiry warning and the silent session refresh, with the two
	// timers, the two notices and the in-flight/failed flags they need. All of
	// that used to sit here among the plugin's other fields, where nothing
	// stopped an unrelated method reaching into it; it is one object with one
	// teardown now. Every dependency below is read at call time, so none of
	// them has to be ready when this runs.
	private readonly sessionRenewal = new SessionRenewal({
		...
		showActionNotice: (message, actionLabel, run) =>
			this.showActionNotice(message, actionLabel, run),
		forgetActionNotice: (notice) => this.actionNotices.delete(notice),
		redrawSettings: () => this.settingsRefresh?.(),
		isReauthInFlight: () => this.reauthInFlight,
		reconnectPrefersWindow: () => this.reconnectPrefersWindow(),
		reconnectFromNotice: () => this.reconnectFromNotice(),
	...
	get sessionRenewalPaused(): boolean {
		return this.sessionRenewal.paused;
	}
	...
	private readonly captureStore = new CaptureStore<PlaudImporterSettings>({
		...
		clearRefreshFailure: () => {
			this.sessionRenewal.clearFailure();
		},
		noteShortLifetimeOnCapture: (token, signInMethod) =>
			this.sessionRenewal.noteShortLifetimeOnCapture(token, signInMethod),
		reconcileSessionExpiryWarning: () =>
			this.sessionRenewal.reconcileExpiryWarning(),
		reconcileSessionRefresh: () => this.sessionRenewal.reconcileRefresh(),
	...
	async onload() {
		...
		this.app.workspace.onLayoutReady(() => {
			...
			if (isLegacyPartition(this.sessionRenewal.signInPartition())) {
	...
	onunload() {
		...
		// Both renewal timers and the notices they own, in one teardown.
		this.sessionRenewal.dispose();
	...
	// ---- Session renewal, delegated ---------------------------------------
	//
	// The subsystem lives in session-renewal.ts. These three stay on the plugin
	// because they are its published surface: the settings tab reaches them
	// through SettingsTabHost, and the capture store reconciles both timers
	// after every credential write. Forwarding here rather than handing either
	// caller the renewal object keeps that object's state private to it.
	...
	/** True when reconcileSessionRefresh would arm renewal for this credential. */
	canRenewCredential(
		token: string,
		signInMethod: SignInMethod = this.settings.signInMethod,
	): boolean {
		return this.sessionRenewal.canRenewCredential(token, signInMethod);
	}

	/** One renewal attempt. Reached only from the debug command. */
	async refreshSessionNow(): Promise<RefreshOutcome> {
		return this.sessionRenewal.refreshNow();
	}
	...
	async reauthenticate(): Promise<ReauthOutcome> {
		...
		if (this.sessionRenewal.refreshInFlight) {
			new Notice(
