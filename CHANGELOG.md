...
- **A sign-in that cannot be saved no longer half-applies.** Saving a captured sign-in writes two things: the credential itself, and the plugin settings that describe it (which regional server your account is on, and which sign-in method you used). The credential was written first, so if the settings write then failed, the two disagreed: the new credential was stored while the settings still described the previous one, and the mismatch survived a restart. That could send your new sign-in to the wrong regional server, or point **Reconnect** at the wrong sign-in window. The settings write happens first now, and the credential is stored only once that write has landed, so a failure changes nothing at all and the sign-in you already had keeps working. The settings write is the one that can fail for ordinary reasons: a vault on read-only storage, a full disk, or a sync client holding the file (issue #86).
- **A capture that failed to save now says so, and says it once.** The message a failed save shows can now promise that nothing was changed, because nothing was. It is also no longer followed by a second message contradicting it: a sign-in that failed for a reason the plugin had already named, such as Plaud rejecting every token it found or the vault refusing the write, drew a further "sign-in closed" notice claiming you had walked away from a sign-in you had in fact completed. Whichever message names the real reason is now the only one shown (issue #86).
- **Clear sign-in and choosing a token by hand now behave the same way.** Both write to the vault, and both could fail the same way a sign-in could. **Clear sign-in** wiped your stored token first and saved afterwards, so a vault it could not write to left you signed out in practice but still signed in on disk, and reported a clean sign-out anyway. Choosing a different token in the picker could likewise leave the dropdown showing a choice that was never saved. Each now saves first and changes nothing if that save fails, and says so instead of reporting success (issue #86).
- **Two things saving your settings at the same time can no longer overwrite each other.** The plugin writes its settings from several places, some of them in the background: noting which regional server your account is on, saving a change you made in settings, storing a sign-in. Nothing kept two of those from being in flight at once, and whichever finished last won, so a change made by one could be quietly undone by the other. Settings writes now happen one at a time, in the order they were asked for (issue #86).
