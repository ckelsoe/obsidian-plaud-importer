...
- **The plugin now tells the truth about Google/Apple (SSO) sessions instead of promising they renew.** Plaud can end a Google or Apple session early, within hours, so the old copy that said these sessions renew in the background for about 30 days was wrong. The sign-in help, the connection status renewal line, and the capture notice no longer make that promise for SSO. When your current session is a Google or Apple sign-in, the status under "Plaud token" shows a short note that these sessions end early, with an expandable "Why this happens and how to fix it" (add a password to your Plaud account and use email sign-in). Email sign-in is unchanged: it still renews for about 30 days, and an email session shows nothing extra.
- **The long sign-in help and browser steps now collapse.** The Sign in section used to open with a wall of text. It now leads with a one-line summary and an expandable "How sign-in works and how long it lasts", and the browser sign-in steps sit behind a "Show the step-by-step" toggle. Same information, one click away, with far less to scroll past.
