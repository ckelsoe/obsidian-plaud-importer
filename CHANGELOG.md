...
- **Signing in now tells you when the problem is your vault, not your token.** Saving a sign-in stores the credential and then writes the plugin's settings, and that second write can fail for ordinary reasons: a vault on read-only storage, a full disk, or a sync client holding the file. From the **Sign in** and **Paste token from clipboard** buttons in settings, that failure showed you nothing at all. The button simply re-enabled, the sign-in looked like it had quietly done nothing, and there was no hint that the vault was what went wrong. Both buttons now say the token could not be saved to this vault and suggest checking that it is writable (issue #86).
